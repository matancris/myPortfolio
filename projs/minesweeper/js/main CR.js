'use strict';
//@CR: Hi Matan, you made a great project, I'll put comments with '@CR' at the beginning,
//you can ctrl+f and search for '@CR' to jump through the different comments.
    ...
    for (var i = 0; i < board.length; i++) {
        ...
        for (var j = 0; j < board[0].length; j++) {
            ...
            if (cell.isMine) {
                ...
                //@CR: I wouldn't put MINE in minesAroundCount, maybe a new variable that you will set to minesAroundCount or MINE
                //and use that variable inside the template string, 
                //something like '...cellMarked(this,${i},${j})">${cellHTML} </td>`
...
function cellClicked(elCell, i, j) {
    ...
    gIsFirstClick = false;
    //@CR: this should be inside the if at line 182, everytime that you click on the board you re-assign false, instead of assigning only once;
...
function cellMarked(elCell, i, j) {
    window.addEventListener('contextmenu', function (elCell) { // makes the contextmenu not to show up
        elCell.preventDefault(); //@CR: elCell is not element but an event.
    }, false);
    //@CR: you could put event listener on the board element and not on the whole window itself.
    //@CR: different approach, send the mouse-event to the function and check if right-click has been clicked, if so event.preventDefault();
    ...
    if (!gBoard[i][j].isShown) {
        if (!gBoard[i][j].isMarked) {
            ...
            //@CR: two line above are repeated, you could move them outside of the function and use el.classList.toggle('marked') and bool = !bool
...
function gameOver(isWin) {
    ...
    if (!isWin) {
        ...
        elSmiley.innerText = '🥳';
        renderModal('win');
        //@CR: same here repeated code, use short-if
        checkBestScore()
    }
}
